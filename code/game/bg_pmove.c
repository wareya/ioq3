...
void PM_AddEvent( int newEvent ) {
	if(newEvent == EV_FIRE_WEAPON)
		BG_AddPredictableEventToPlayerstate( newEvent, finalTime, pm->ps ); // finalTime get masked by 0xFF if sent over network, we use this on the client to tell if this was generated by the server or by prediction
	else
...
//		pm->ps->velocity[2] = 0;
	...
	//new idea: change it to retain horizontal velocity instead?
	//new idea: do it the old way but fix slope logic?
	if(pm_overbouncefix)
	{
		float tempdot2;
		vec3_t tempvec;
		...
		PM_ClipVelocity (pm->ps->velocity, pml.groundTrace.plane.normal, pm->ps->velocity, OVERCLIP );
		// if it's a slope relative to our own motion, normalize absolute velocity
		if(tempdot2 > 0.7f)
		{
			VectorNormalize(pm->ps->velocity);
			VectorScale(pm->ps->velocity, vel, pm->ps->velocity);
